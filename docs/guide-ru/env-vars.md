...
| APP_SENTRY_DSN      | string | нету                  | Токен для отправки ошибок в Sentry ([подробности](https://docs.sentry.io/error-reporting/quickstart/?platform=node#configure-the-sdk)) |
...
| APP_BLOCKCHAIN_VERSION          | string | current               | Версия блокчейна и часть имени бинарного файла, который будет запущен                   |
...
| APP_BLOCKCHAIN_LOGS_TO_STDOUT   | bool   | true                  | Транслировать ли логи запущенной ноды в систему логирования                             |
| APP_BLOCKCHAIN_NOTIFY_NEW_BLOCK | bool   | true                  | Генерировать ли ивент о том, что пришел новый блок, если в логах были обнаружены данные |
